...
(* If you use the version available from the lab machines, the
   version is 4.05.0 ***)
...
let tlmin (xs: float list): float option =
    ...
    | [] -> None
    | head::tail ->
        let rec recursion (currentMinimum: float) (l: float list): float =
            match l with
            | [] -> currentMinimum
            | head::tail ->
                if head < currentMinimum then
                    recursion head tail
                else
                    recursion currentMinimum tail
        in
            Some (recursion head tail)
...
let rec product (xs: float list): float =
    ...
    | head::tail ->
        if head = 0. then
            ...
            match (tlmin xs) with
            | None -> infinity    (* Should not reach here *)
            | Some f -> raise (NegativeInList f)
        else
            head *. (product tail)
...
let try_product (xs: float list): string =
    ...
    | ProductIsZero -> "0."
    | (NegativeInList f) -> "List contains a negative number; minimum is " ^ (string_of_float f)
...
(*
    They consume the same amount of time.
    
    In f4, the two branches of the if-condition statement are both evaluated,
    no matter what value z has.
*)
...
let s = ref (UN(fun () -> fib 14))
let t = ref (UN(fun () -> fib 41))
...
(*
    ...
    During the call of m2, both values of fib 14 and fib 41 are evaluated and stored.
    Then during the call of m1, since fib 41 was evaluated, the function would directly return the cached value,
    thus n2 is faster.
*)

let m2' = f4_cbn s t 1
(*
    It is for the same reason: both values of fib 14 and fib 41 are evaluated and stored in the call of m2.
    during the call of m2', since fib 14 was evaluated, the function would directly return the cached value.
*)
