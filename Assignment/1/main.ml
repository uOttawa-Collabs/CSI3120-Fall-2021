...
let fullname : string = name ^ " Wang"

(* 1c. Create a string containing your email address *)
let email : string = "foo@example.com"
...
let rec prob2d (x: float) (y: int): char =
  if x = float_of_int y then prob2c y else prob2d x y


(* Problem 3 - Explain why each of 3a and 3b will not compile.  Note
   that there are 2 distinct problems in 3b.  Use either of strings exp3a,
   exp3b1, and exp3b2 for your answers. use only one of the strings 
   to respond. Then change the code in some
...
let exp3a: string = "Variable y is of type float, but operator (*) must have int operand."
...
let exp3b1 : string = "k - 1 must be grouped by parentheses, or the expresion will become x * (exp x k) - 1."

let exp3b2 : string = "Did not define types for x and k."
...
let exp4 : string = "Function does not define a base case, thus leading to infinity recursion."
...
let is_palindrome (s: string): bool =
    if String.length s < 2 then
        ...
        let rec recursion (s: string) (i: int): bool =
            if i > ((String.length s) / 2) then
                true
            else
                if (s.[i] == s.[(String.length s) - i - 1]) then
                    recursion s (i + 1)
                else
                    false
        in
