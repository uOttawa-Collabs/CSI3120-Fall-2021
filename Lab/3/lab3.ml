...
let rec max_of_list (lst: int list): int option =
    ...
    | head::[] -> Some head
    | head::tail ->
        let max_tail = (max_of_list tail) in
            if (max_option2 (Some head) max_tail = (Some head)) then
                Some head
            else
                max_tail
...
(*
    ...
    Function sum adds up all elements in the list.
    Function prod multiplies up all elements in the list.
    
    Function reduce takes the first element in the list
    as an initial "total", then applies an operation between
    the "total" and each element (other than the first one)
    in the list, then update the "total" to the result of
    the operation.
*)

(*
    In order to trace the reduce function, we need to create
    an monomorphic version of it. 
*)
let rec reduce_m (f:int -> int -> int) (u: int) (xs: int list): int =
  match xs with
  | [] -> u
  | hd::tl -> f hd (reduce_m f u tl);;
  
let sum_m xs = reduce_m (fun x y -> x+y) 0 xs
let prod_m xs = reduce_m (fun x y -> x*y) 1 xs

(*
    # trace reduce_m;;
*)
let mysum = sum_m [2;5;6]
(*
    # let mysum = sum_m [2;5;6];;
    reduce_m <-- <fun>
    reduce_m --> <fun>
